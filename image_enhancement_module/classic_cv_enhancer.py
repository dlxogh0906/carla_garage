"""Standalone copy of the classic_cv image enhancer.

This file mirrors the active enhancer used by:
  carla_garage/src/garage_ext/modules/image_enhancer/classic.py

The only difference is that this standalone copy does not depend on the
garage_ext registry decorator, so it can be read or imported independently.
"""
...
def _analyze(img_bgr: np.ndarray) -> str:
  ...
  low_light = (
      0.60 * _clamp01((90.0 - bm) / 50.0)
      + 0.25 * _clamp01((170.0 - p90) / 80.0)
      + 0.15 * _clamp01((45.0 - contrast) / 25.0)
  )
  over_exp = (
      0.50 * _clamp01((bm - 180.0) / 50.0)
      + 0.30 * _clamp01((p10 - 110.0) / 60.0)
      + 0.20 * _clamp01((45.0 - contrast) / 25.0)
  )
  haze = (
      0.40 * _clamp01((45.0 - contrast) / 25.0)
      + 0.30 * _clamp01((70.0 - sat_mean) / 40.0)
      + 0.30 * _clamp01((dc_mean - 90.0) / 80.0)
  )
  blurry = _clamp01((120.0 - sharpness) / 100.0)

  scores = {
      "low_light": low_light,
      "over_exposed": over_exp,
      "haze": haze,
      "blurry": blurry,
  }
  best = max(scores, key=scores.__getitem__)
  return best if scores[best] >= 0.35 else "normal"
...
def _process(img: np.ndarray, mode: str) -> np.ndarray:
  ...
  if mode == "haze":
    return _unsharp(
        _increase_sat(_clahe(_contrast_stretch(img), clip=2.0), scale=1.20),
        sigma=1.0,
        amount=0.8,
    )
  if mode == "blurry":
...
class ClassicCVEnhancer:
  """Adaptive multi-mode enhancer.

  The enhancer first classifies each frame as low-light, over-exposed, haze,
  blurry, or normal, then applies the matching traditional CV correction.
  """
