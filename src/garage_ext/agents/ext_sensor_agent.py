...
class ExtSensorAgent(SensorAgent):
    ...
    def setup(self, path_to_conf_file, route_index=None, traffic_manager=None):
        ...
        self._image_enhancer = (
            registry_build("image_enhancer", self._ext_cfg.image_enhancer,
        ...
        self._compare_root = (
            pathlib.Path(os.environ.get("SAVE_PATH", ""))
            if os.environ.get("SAVE_PATH") else None
        )
    ...
    def _save_compare(self, frames: list) -> None:
        ...
        for key, bgr_orig, bgr_enh in frames:
            ...
            cv2.putText(orig_labeled, f"ORIGINAL [{label}]", (10, 30),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
            cv2.putText(enh_labeled, f"ENHANCED [{label}]", (10, 30),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
