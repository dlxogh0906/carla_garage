#!/bin/bash
# Run TF++ + rear ClassicCV 8-meta-action VLA on dev10 for quantization methods:
#   1. BF16 raw Qwen3-VL, transformers
#   2. W8A8-INT8, vLLM
#   3. AWQ-W4A16, vLLM
#   4. GPTQ-W4A16, vLLM
#   5. MixedGPTQ-W4W8A16, vLLM
#   6. AWQScaledGPTQ-W4A16, vLLM
...
SUITE_ROOT=${SUITE_ROOT:-${RESULT_ROOT}/rear_classiccv_8meta_quant_methods_suite_$(date +%Y%m%d_%H%M%S)}
RUNS=${RUNS:-bf16_transformers,w8a8_int8_vllm,awq_w4a16_vllm,gptq_w4a16_vllm}
SUITE_LOCK=${SUITE_LOCK:-/tmp/rear_classiccv_8meta_quant_methods_suite.lock}
...
MODEL_MIXED=${MODEL_MIXED:-/mnt/2/pretrained_models/Qwen3-VL-8B-Instruct-MixedGPTQ-W4W8A16}
MODEL_ACTAWARE_MIXED=${MODEL_ACTAWARE_MIXED:-/mnt/2/pretrained_models/Qwen3-VL-8B-Instruct-ActAwareMixedGPTQ-W4W8A16-n8}
MODEL_AWQ_SCALED_GPTQ=${MODEL_AWQ_SCALED_GPTQ:-/mnt/2/pretrained_models/Qwen3-VL-8B-Instruct-AWQScaledGPTQ-W4A16-n64}
...
META_DASHBOARD_MAKE_VIDEO=${META_DASHBOARD_MAKE_VIDEO:-1}
META_DASHBOARD_VIDEO_FPS=${META_DASHBOARD_VIDEO_FPS:-6}
export META_DASHBOARD_EVERY_N_STEPS=${META_DASHBOARD_EVERY_N_STEPS:-3}
export META_DASHBOARD_EXPLANATION_HOLD_STEPS=${META_DASHBOARD_EXPLANATION_HOLD_STEPS:-20}
...
exec 9>"$SUITE_LOCK"
if ! flock -n 9; then
  echo "[lock] another rear ClassicCV 8meta quant-method suite is running: ${SUITE_LOCK}" >&2
  exit 2
fi
...
make_meta_dashboard_videos() {
  local out_dir="$1"
  local dashboard_root="${out_dir}/meta_dashboard"
  [ "$SAVE_META_DASHBOARD" = "1" ] || return 0
  [ "$META_DASHBOARD_MAKE_VIDEO" = "1" ] || return 0
  [ -d "$dashboard_root" ] || return 0
  if ! command -v ffmpeg >/dev/null 2>&1; then
    log "[MetaDash] ffmpeg not found; skip dashboard mp4"
    return 0
  fi

  local frame_dir
  while IFS= read -r frame_dir; do
    compgen -G "${frame_dir}/*.png" >/dev/null || continue
    log "[MetaDash] making video: ${frame_dir}/dashboard.mp4"
    env -u LD_LIBRARY_PATH ffmpeg -y \
      -nostdin \
      -framerate "$META_DASHBOARD_VIDEO_FPS" \
      -pattern_type glob \
      -i "${frame_dir}/*.png" \
      -vf "pad=ceil(iw/2)*2:ceil(ih/2)*2" \
      -c:v libx264 \
      -pix_fmt yuv420p \
      "${frame_dir}/dashboard.mp4" \
      >>"${out_dir}/eval.log" 2>&1 || log "[MetaDash] video failed: ${frame_dir}"
  done < <(find "$dashboard_root" -mindepth 1 -maxdepth 1 -type d | sort)
}
...
run_eval() {
  ...
  while [ $attempt -lt "$MAX_RETRIES" ]; do
    ...
    if [ $exit_code -eq 0 ]; then
      make_meta_dashboard_videos "$out_dir"
      return 0
    fi
...
run_transformers_case "bf16_transformers" "BF16-transformers" "bf16" "$MODEL_RAW"
run_vllm_case \
  "w8a8_int8_vllm" \
  "W8A8-INT8-vLLM" \
  "w8a8-int8" \
  "$MODEL_W8A8" \
  "Qwen3-VL-8B-W8A8-INT8" \
  "compressed-tensors"
...
run_vllm_case \
  ...
  "gptq-w4a16" \
  "$MODEL_GPTQ" \
  "Qwen3-VL-8B-GPTQ-W4A16" \
  "compressed-tensors"

run_vllm_case \
  "mixed_gptq_w4w8a16_vllm" \
  "MixedGPTQ-W4W8A16-vLLM" \
  "mixed-gptq-w4w8a16" \
  "$MODEL_MIXED" \
  "Qwen3-VL-8B-MixedGPTQ-W4W8A16" \
  "compressed-tensors"

run_vllm_case \
  "actaware_mixed_gptq_w4w8a16_vllm" \
  "ActAwareMixedGPTQ-W4W8A16-vLLM" \
  "actaware-mixed-gptq-w4w8a16" \
  "$MODEL_ACTAWARE_MIXED" \
  "Qwen3-VL-8B-ActAwareMixedGPTQ-W4W8A16" \
  "compressed-tensors"

run_vllm_case \
  "awq_scaled_gptq_w4a16_vllm" \
  "AWQScaledGPTQ-W4A16-vLLM" \
  "awq-scaled-gptq-w4a16" \
  "$MODEL_AWQ_SCALED_GPTQ" \
  "Qwen3-VL-8B-AWQScaledGPTQ-W4A16" \
  "compressed-tensors"

"$LEADERBOARD_PYTHON" "${SCRIPT_DIR}/tools/summarize_qwen_dev10_quant_suite.py" \
  --suite-root "$SUITE_ROOT" \
  --output-prefix "${SUITE_ROOT}/rear_classiccv_8meta_quant_methods_metrics"

log "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
log "Done"
log "CSV: ${SUITE_ROOT}/rear_classiccv_8meta_quant_methods_metrics.csv"
log "MD : ${SUITE_ROOT}/rear_classiccv_8meta_quant_methods_metrics.md"
log "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
