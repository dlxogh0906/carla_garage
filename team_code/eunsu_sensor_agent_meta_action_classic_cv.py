"""
sensor_agent_meta_action_classic_cv.py — TF++ + Meta-action VLA + Classic CV 이미지 보정

sensor_agent_meta_action_3rd_try 방법론 (TTC=3.0s, 20step, 전방 카메라만) +
ClassicCVEnhancer (low_light / over_exposed / haze / blurry / normal 적응형 보정)

변경 내용 (AWN 대비):
  - AllWeatherNet 제거 → ClassicCVEnhancer (checkpoint 불필요, 순수 CV)
  - enhance는 BGR frame에 적용 후 RGB 변환 (ClassicCV 인터페이스: BGR→BGR)
  - VLM 입력: ClassicCV 보정된 RGB 이미지
"""
...
class SensorAgent(autonomous_agent.AutonomousAgent):
  ...
  def setup(self, path_to_conf_file, route_index=None, traffic_manager=None):
    ...
    if use_classic_cv:
      ...
      print('[ClassicCV] Image enhancement enabled.')
    else:
      self.enhancer = None
    ...
    # ----------------------------------------------------------------
    # Meta-action VLA: Qwen2.5-VL — TTC 기반 8-class 메타-액션 플래너
    # ----------------------------------------------------------------
    ...
    from eunsu_meta_action_vla import MetaActionVLAPlanner  # pylint: disable=import-outside-toplevel
    ...
    self._dashboard_vis = None  # visualize_frame 함수 (대시보드 시각화)
    try:
      from dashboard import visualize_frame  # pylint: disable=import-outside-toplevel
      self._dashboard_vis = visualize_frame
      print('[MetaDash] Dashboard visualization enabled.')
    except Exception as e:  # pylint: disable=broad-except
      print(f'[MetaDash] Dashboard import failed: {e}')

    # META_DASHBOARD_PATH: ORIG_VIS_PATH 방식과 동일하게 route별 서브폴더 생성
    # leaderboard_evaluator가 route_index를 넘기지 않으므로 path_to_conf_file로 루트명 추출
    ...
    self.orig_vis_path = None  # meta_action 에서는 orig_vis 미사용

    # Path to where visualizations and other debug output gets stored.
    # leaderboard_evaluator may not pass route_index for these custom route
    # configs, so derive a stable route folder from path_to_conf_file as a
    # fallback. Without this, VLM calls appear in eval.log but runtime CSVs stay
    # empty because qwen_intervention.jsonl is never opened.
  ...
  def tick(self, input_data):
    ...
    for camera_pos in ['front']:
      ...
      # Store ClassicCV-enhanced full-res RGB for Meta-action VLA
  ...
  def run_step(self, input_data, timestamp, sensors=None):  # pylint: disable=locally-disabled, unused-argument
    ...
    # ----------------------------------------------------------------
    # Meta-action VLA: Qwen2.5-VL — TTC 기반 8-class 메타-액션
    #
    # 1. bbs_vehicle_coordinate_system으로 TTC 계산
    # 2. TTC < ttc_threshold → VLM 비동기 트리거
    ...
    if _ttc_danger and self._meta_rgb_np is not None:
      self.meta_action.request_guidance(self._meta_rgb_np, self.step)

    if self.config.use_controller_input_prediction and _ttc_danger:
      _multiplier = self.meta_action.get_speed_multiplier()
      pred_target_speed_scalar = pred_target_speed_scalar * _multiplier
    else:
      _multiplier = 1.0
    _meta_result = self.meta_action.get_last_result()
    ...
    if self.config.inference_direct_controller and self.config.use_controller_input_prediction:
      ...
      # ── 대시보드 시각화 (매 5스텝, META_DASHBOARD_PATH가 있을 때만) ────
      if self._dashboard_vis is not None and self._meta_dash_path is not None and self.step % 5 == 0:
  ...
  def _save_meta_dashboard(
    ...
    """대시보드 이미지를 META_DASHBOARD_PATH/<route>/ 에 저장."""
    ...
    try:
      ...
      # TTC → danger score (0~1): TTC=0 → 1.0, TTC=ttc_threshold → 1.0, TTC=6s → 0.5
  ...
  def _log_qwen_step(self, ttc, ttc_danger, speed_before, speed_after, multiplier, meta_result, control):
    ...
    new_completed_request = request_id is not None and request_id != self._last_logged_qwen_request_id
    entry = {
        'experiment': 'eunsu_8meta_action_classiccv',
        ...
        'vlm_called': bool(new_completed_request),
        'vlm_trigger': 'ttc' if ttc_danger else 'none',
    ...
    if meta_result.get('benchmark') is not None:
      entry['qwen_benchmark'] = meta_result['benchmark']
    if new_completed_request:
      self._last_logged_qwen_request_id = request_id
    self._qwen_log_f.write(json.dumps(entry) + '\n')
