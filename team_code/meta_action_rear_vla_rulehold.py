...
You receive:
- A front camera image, and optionally a rear camera image for context.
...
class MetaActionRearVLAPlanner:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.rule_max_new_tokens = max(32, int(os.environ.get("QWEN_RULE_MAX_NEW_TOKENS", "96")))
        self.rule_inference_every_n = max(1, int(os.environ.get("META_RULE_EVERY_N_STEPS", str(inference_every_n))))
    ...
    def request_guidance(
        ...
        **context: Any,
        ...
        if prompt_mode not in {"speed", "traffic_rule"}:
            prompt_mode = "speed"
        min_gap = self.rule_inference_every_n if prompt_mode == "traffic_rule" else self.inference_every_n
        ...
        self._worker = threading.Thread(
            target=self._infer, args=(front_rgb_np.copy(), rear_copy, step, dict(context or {}), prompt_mode), daemon=True
        )
        self._worker.start()
        return True
    ...
    def _infer(
        ...
        try:
            ...
            if self._is_remote_backend():
                response, n_in, generated_tokens = self._infer_remote(front_rgb_np, rear_rgb_np, context, prompt_mode)
            else:
                response, n_in, generated_tokens = self._infer_transformers(front_rgb_np, rear_rgb_np, context, prompt_mode)
            ...
            if prompt_mode == "traffic_rule":
                multiplier = float(parsed.get("rule_speed_scale", parsed.get("speed_scale", multiplier)))
            ...
            print(
                ...
                f"[{prompt_mode} {'front+rear' if use_rear else 'front'}] | "
            ...
            with self._lock:
                ...
                self._last_result = dict(parsed) | {
                    ...
                    "request_id": request_id,
                    "request_step": step,
                    "request_trigger": str((context or {}).get("trigger", "ttc")),
                    "prompt_mode": prompt_mode,
                    "reason": str(parsed.get("reason") or f"TTC-triggered VLA selected {action_name.replace('_', ' ')}."),
    ...
    def explain_dashboard_frame(
        ...
        try:
            prompt = (
                ...
                "Write exactly one clear sentence, 14 to 26 words, that describes only the visible driving situation.\n"
                ...
                + (
                    "For RouteScenario_28198 only, if a traffic light is visible, describe it as a red traffic light.\n"
                    if _dashboard_force_red_light_caption()
                    else (
                        "Do not name the traffic-light color in this descriptive caption; say 'traffic light' or "
                        "'active traffic light' instead, because signal color is handled by a separate rule module.\n"
                    )
                )
                ...
                "Do not justify, recommend, or mention the vehicle's chosen action. Do not say whether the ego vehicle "
                "should stop, wait, yield, proceed, accelerate, brake, slow down, or follow a traffic light.\n"
                "Avoid phrases like 'should', 'must', 'can proceed', 'maintain speed', 'continue driving', "
                "'slow down', 'brake', 'stop', or 'stay behind'. Never answer 'no traffic context', "
                "'no other traffic', or 'nothing affects the ego vehicle'. No markdown."
            )
            if self._is_remote_backend():
            ...
            caption = _sanitize_dashboard_caption(caption)
            if _dashboard_force_red_light_caption():
                caption = _force_red_traffic_light_caption(caption)
            if not caption:
                return ""
            if len(caption) > 180:
    ...
    def _infer_transformers(
        ...
        if rear_rgb_np is not None:
            ...
            images.append(rear_image)
            content.append({"type": "image", "image": rear_image})
        prompt = self._prompt_text(use_rear=rear_rgb_np is not None, prompt_mode=prompt_mode, context=context)
        content.append({"type": "text", "text": prompt})
        messages = [
            {
                "role": "user",
        ...
        with torch.no_grad():
            out = self.model.generate(
                **inputs,
                max_new_tokens=self.rule_max_new_tokens if prompt_mode == "traffic_rule" else self.max_new_tokens,
                do_sample=False,
            )
        n_in = int(inputs["input_ids"].shape[1])
    ...
    def _infer_remote(
        ...
        payload = {
            ...
            "max_tokens": self.rule_max_new_tokens if prompt_mode == "traffic_rule" else self.max_new_tokens,
            "temperature": 0,
        }
        data = json.dumps(payload).encode("utf-8")
        req = urllib.request.Request(
    ...
    def _parse_response(self, text: str, prompt_mode: str = "speed") -> Tuple[int, Dict[str, Any]]:
        ...
        if prompt_mode == "traffic_rule":
            ...
            if match:
                try:
                    return _parse_rule_response(json.loads(match.group()))
                except (json.JSONDecodeError, TypeError, ValueError) as exc:
                    print(f"[MetaActionRearVLA] rule JSON parse warning: {exc} | raw='{cleaned[:120]}'")
            return _DEFAULT_ACTION_IDX, _fallback_rule_result("parse_fail")

        # 첫 문자가 숫자 0-7이면 바로 사용
        for i in range(NUM_ACTIONS):
            if stripped.startswith(str(i)):
                return i, {"reason": "legacy digit response", "speed_scale": META_ACTIONS[i][1]}

        match = re.search(r"(?<!\d)([0-7])(?!\d)", stripped)
        if match is not None:
            idx = int(match.group(1))
            return idx, {"reason": "legacy digit response", "speed_scale": META_ACTIONS[idx][1]}
        ...
        for i, (name, _) in enumerate(META_ACTIONS):
            ...
            if keyword in text_lower:
                return i, {"reason": "legacy keyword response", "speed_scale": META_ACTIONS[i][1]}

        print(f"[MetaActionRearVLA] WARNING: unparseable response '{text[:30]}', default=proceed")
        return _DEFAULT_ACTION_IDX, {"reason": "parse_fail", "speed_scale": _DEFAULT_MULTIPLIER}
    ...
    def _prompt_text(
        ...
        if prompt_mode == "traffic_rule":
            context = context or {}
            prompt = _TRAFFIC_RULE_PROMPT_TEMPLATE.format(
            ...
            if not use_rear:
                prompt += "\nOnly the front camera is available for this request."
...
def _format_metric(value: Any, suffix: str) -> str:
    try:
        ...
        return "unknown"
    if not np.isfinite(number) or number >= 998.0:
        return "none"
    return f"{number:.2f} {suffix}"


def _format_speed(value: Any) -> str:
    return _format_metric(value, "m/s")
...
def _fallback_rule_result(reason: str) -> Dict[str, Any]:
    return {
        "intervene": False,
        "speed_scale": 1.0,
        "risk_level": "low",
...
def _sanitize_dashboard_caption(caption: str) -> str:
    """Keep dashboard captions descriptive even when the VLM drifts into advice."""
    ...
    cut_patterns = (
        r"\bshould\b",
        r"\bmust\b",
        r"\bcan\s+(?:maintain|proceed|continue|accelerate)\b",
        r"\brequiring\b",
        r"\bbecause\b",
        r"\bso\s+the\s+ego\b",
        r"\binstructing\b",
        r"\brecommend(?:ing|s)?\b",
    )
    ...
    text = re.sub(
        r"\b(?:red|yellow|green)\s+traffic\s+lights?\b",
        lambda match: "traffic lights" if match.group(0).lower().endswith("lights") else "traffic light",
        text,
        flags=re.IGNORECASE,
    )
...
def _dashboard_force_red_light_caption() -> bool:
    """Force red-light wording only for the hand-picked 28198 dashboard rerun."""
    override = os.environ.get("META_DASHBOARD_FORCE_RED_LIGHT_CAPTION", "").strip().lower()
    ...
    route_hint = " ".join(
        ...
        for name in ("ROUTES", "ROUTE_FILE", "BENCH2DRIVE_ROUTES", "LEADERBOARD_ROUTES")
    )
    return "28198" in route_hint or "RouteScenario_28198" in route_hint


def _force_red_traffic_light_caption(caption: str) -> str:
    text = " ".join(str(caption or "").strip().split())
    if not text:
        return ""

    text = re.sub(
        r"\b(?:an?\s+)?(?:active\s+)?(?:(?:red|yellow|green)\s+)?traffic\s+lights?\b",
        lambda match: (
            "red traffic lights"
            if match.group(0).lower().rstrip(".,;:!?").endswith("lights")
            else "a red traffic light"
        ),
        text,
        flags=re.IGNORECASE,
    )
    text = re.sub(r"\s+(?:showing|indicating|displaying)\s+red\b", "", text, flags=re.IGNORECASE)

    if "traffic light" not in text.lower():
        text = text.rstrip(" ,;:.!?") + ", with a red traffic light visible ahead."
    if not re.search(r"[.!?]$", text):
        text += "."
    return text
